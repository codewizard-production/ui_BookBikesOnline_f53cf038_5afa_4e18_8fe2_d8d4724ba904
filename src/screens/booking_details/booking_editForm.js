...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], bookingId, booking, numfields, mapItem;

        booking = row['booking'];
        bookingId = row['booking'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) bookingId = rslt.id;
                            
                            const mapPropKey = booking.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "BookingID", value: parseInt(bookingId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateBooking(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'booking');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Booking is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
