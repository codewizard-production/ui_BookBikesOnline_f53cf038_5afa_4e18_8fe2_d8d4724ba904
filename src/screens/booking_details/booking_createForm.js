...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, bookingId, numfields;
        const mapItems = MapItems;

        let booking = row['booking'];

        numfields = Helper.GetAllNumberFields(booking);
        if (numfields.length > 0) Helper.UpdateNumberFields(booking, numfields);


        // Add Or Update Booking
        rslt = await Support.AddOrUpdateBooking(booking, []);
        if (rslt.status) {
            bookingId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Booking and navigation entity if it is deos not exist
            let navItem = booking.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "BookingID", value: parseInt(bookingId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateBooking(data);
        ...
        global.AlertPopup("success", "Booking is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
